...
> **注意**：也可以使用 `scripts/wechat-claude.sh` 作为启动入口，它直接用 `bun run` 运行（避免 Bun `--compile` 在 macOS 上的 segfault 问题）。`~/.local/bin/wechat-claude` 已软链接到该脚本。
