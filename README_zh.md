...
│
├── client/claude-code-adapter/  # Claude Code 适配器 (Node.js/TypeScript → wechat-claude 二进制)
│   └── src/
...
│
├── scripts/
│   └── build.sh                 # 一键构建所有二进制 (gateway + wechat-claude)
│
└── docs/
```
...
# 一键构建所有组件（gateway + wechat-claude 二进制）
./scripts/build.sh
...
Claude Code Adapter 是独立的 agent（Node.js/TypeScript），它连接到 Claude Code 而不是 Hermes。构建后编译为独立二进制，无需 Node.js 环境：

```bash
# 构建（scripts/build.sh 已包含，也可单独编译）：
