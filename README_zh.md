...
`CLAUDE_BINARY_PATH` 在 launchd 模式下是必需的——指向 `claude` CLI 二进制的位置。launchd 的 PATH 环境很精简，不设置则 adapter 无法找到 Claude Code。
...
# 运行（通过 PATH 或 CLAUDE_BINARY_PATH 环境变量找到 claude）
